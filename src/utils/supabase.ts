...
-- 5. Automatic Welcome Email Trigger for Resend
...
BEGIN
  ...
  IF user_email IS NOT NULL THEN
    PERFORM net.http_post(
      url := 'https://qnbpenvudqrngbxelvnx.supabase.co/functions/v1/send-email',
      headers := jsonb_build_object('Content-Type', 'application/json'),
      body := jsonb_build_object('type', 'welcome', 'to', user_email, 'name', user_name)
    );
  END IF;
  RETURN NEW;
