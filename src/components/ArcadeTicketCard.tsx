  ...
  if (isLoading) {
    return (
      <div className="w-full h-32 sm:h-36 flex shadow-sm rounded-2xl overflow-hidden animate-pulse relative bg-stone-200">
        <div className="flex-1 bg-white border-[3px] border-stone-800 m-2 rounded-xl p-4 flex items-center gap-4 relative">
        ...
        <div className="w-12 shrink-0 bg-stone-50 m-2 ml-0 rounded-r-xl border-[3px] border-stone-800 flex items-center justify-center">
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute left-[-12px] top-1/2 -translate-y-1/2 w-8 h-8 bg-stone-100 rounded-full z-20 border-[3px] border-stone-800 transition-colors group-hover:bg-stone-200" />
        
        {/* Right Notch */}
        <div className="absolute right-[-12px] top-1/2 -translate-y-1/2 w-8 h-8 bg-stone-100 rounded-full z-20 border-[3px] border-stone-800 transition-colors group-hover:bg-stone-200" />

        {/* Inner White Body with thick dark border */}
        <div className="flex-1 bg-white rounded-xl flex relative border-[3px] border-stone-800 transition-colors group-hover:bg-stone-50">
          
          {/* Main Content Area */}
          <div className="flex-1 p-3 sm:p-4 flex items-center gap-4 sm:gap-6 z-10 pl-6 sm:pl-8">
            {/* Avatar */}
            <div className="w-14 h-14 sm:w-16 sm:h-16 shrink-0 rounded-xl bg-stone-100 border-2 border-stone-800 shadow-sm flex items-center justify-center">
            ...
            <div className="flex-1 flex flex-col items-start justify-center gap-1 sm:gap-2 min-w-0">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-black font-display tracking-tight text-stone-800 uppercase leading-none truncate w-full">
                {child.name}
              </h3>
              <div className="flex flex-row items-center gap-1.5 sm:gap-3">
                <span className="text-[10px] sm:text-xs font-bold text-stone-800 bg-stone-100 px-2 sm:px-3 py-0.5 rounded-md border-2 border-stone-800 whitespace-nowrap shadow-sm">
                  LVL {child.level}
                </span>
            ...
            {/* Barcode Section */}
            <div className="flex flex-col items-center justify-center shrink-0 pr-2 sm:pr-6 border-l-2 border-transparent sm:border-stone-100 sm:pl-6">
              <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-stone-800 mb-1">ADMIT ONE</span>
              ...
              <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-stone-800">ADMIT ONE</span>
            </div>
          </div>

          {/* Sideways Serial Number */}
          <div className="w-10 sm:w-12 shrink-0 border-l-[3px] border-stone-800 flex items-center justify-center bg-stone-50 z-10 pr-3 sm:pr-4">
