...
// Supabase Edge Function — AI Reward Chart Scanner
//
...
// Then deploy:
//   supabase functions deploy scan-reward-chart
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fetch child's available rewards
    ...
    if (rewardsError || !rewards) {
      return new Response(JSON.stringify({ error: 'Failed to fetch rewards for child' }), {
    ...
    const rewardListText = rewards
      .map((r, idx) => `Row ${idx + 1} (rewardId="${r.id}"): "${r.title}"`)
      .join('\n');

    const prompt = `You are analysing a photograph of a printed children's reward chart.

The chart has these reward rows in order from top to bottom:
${rewardListText}

The chart has 7 columns for days of the week: Mon=dayIndex 0, Tue=1, Wed=2, Thu=3, Fri=4, Sat=5, Sun=6.

Each cell has a hollow star outline. When the child claimed that reward on that day, they COLOURED IN the star with a pen or crayon (the star appears filled/coloured).
...
Example format:
{"detections":[{"rewardId":"abc","rewardTitle":"Ice Cream","dayIndex":0,"detected":true,"confidence":0.95}]}`;
    ...
    let parsedResult: { detections: RewardDetection[] };
    try {
    ...
    const detections: RewardDetection[] = (parsedResult?.detections ?? []).filter(
      (d): d is RewardDetection =>
    ...
    console.error('scan-reward-chart function error:', err);
